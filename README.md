...
| $ux$, $uy$ | 流体速度场 |
| $\rho$ | 由相场变量映射得到的可视化密度 |
| $\rho_{lbm}$ | 由 LBM 分布函数求和得到的原始流体密度 |
...
| 符号 | 对应代码项 | 中文含义 |
|---|---|---|
| $T^{n+1}$ | `next[p]` | 下一时间步局部温度 |
| $T^n$ | `temp[p]` | 当前时间步局部温度 |
| $S_{\mathrm{diff}}$ | `THERMAL_DIFF * lap` | 热扩散项，表示温度场的空间扩散 |
| $S_{\mathrm{adv}}^T$ | `adv` | 温度对流项，表示速度场对温度的输运 |
| $S_{\mathrm{latent}}$ | `cooling` | 潜热冷却项，表示相变过程对温度场的冷却作用 |
| $D_T$ | `THERMAL_DIFF` | 热扩散系数 |
| $C_T$ | `THERMAL_ADV` | 温度对流系数 |
| $L_c$ | `LATENT_COOLING` | 潜热冷却强度 |
| $T_{\mathrm{sat}}$ | `T_SAT` | 饱和温度 |
| $T_{\mathrm{wall}}$ | `T_WALL` | 底部加热壁面温度 |
| $T_{\mathrm{top}}$ | `T_TOP` | 顶部冷边界温度 |
| $T_{\mathrm{upwind}}^n$ | `temp[id(xu, yu)]` | 迎风方向温度 |
| $I(\phi)$ | `interfaceIndicator(phi[p])` | 界面指示函数，用于限制潜热冷却主要发生在气液界面附近 |
...
| 符号 | 对应代码项 | 中文含义 |
|---|---|---|
| $f_i$ | `f[p * Q + k]` | 第 $i$ 个离散速度方向上的分布函数 |
| $f_i^{eq}$ | `feq(k, rho, ux, uy)` | 第 $i$ 个方向的平衡分布函数 |
| $w_i$ | `w[k]` | D2Q9 模型中第 $i$ 个方向的权重系数 |
| $\mathbf{e}_i$ | `(cx[k], cy[k])` | D2Q9 模型中第 $i$ 个方向的离散速度向量 |
| $e_{ix}$ | `cx[k]` | 第 $i$ 个离散速度方向在 $x$ 方向上的分量 |
| $e_{iy}$ | `cy[k]` | 第 $i$ 个离散速度方向在 $y$ 方向上的分量 |
| $\rho$ | `rho` | 由分布函数求和得到的局部流体密度 |
| $\mathbf{u}$ | `(ux[p], uy[p])` | 宏观速度向量 |
| $u_x$ | `ux[p]` | 宏观速度在 $x$ 方向上的分量 |
| $u_y$ | `uy[p]` | 宏观速度在 $y$ 方向上的分量 |
...
| 符号 | 对应代码项 | 当前值 | 中文含义 |
|---|---|---:|---|
| $\rho_0$ | `RHO0` | 1.0 | 基准密度，用于近似不可压流动计算和受力修正 |
| $\tau$ | `TAU` | 0.78 | LBM 松弛时间，影响流体黏性和数值稳定性 |
| $\Omega$ | `OMEGA` | $1 / \tau$ | 碰撞频率，对应 BGK 碰撞项中的松弛强度 |
| $U_{\max}$ | `VMAX` | 0.10 | 速度限制上限，用于防止局部速度过大导致数值发散 |
...
当 $\phi ≈ -1$ 时， $alpha_v ≈ 1$，表示气相区域；当 $\phi ≈ 1$ 时， $alpha_v ≈ 0$，表示液相区域。
...
需要注意的是， $\rho$ 主要用于可视化气液分布； $\rho_{lbm}$ 是由 LBM 分布函数求和得到的原始流体密度。
...
```
其中， $D_T$、 $C_T$ 和 $L_c$ 分别对应代码中的 `THERMAL_DIFF`、`THERMAL_ADV` 和 `LATENT_COOLING`。
...
其中， $\Delta T_1$、 $\Delta T_2$ 和 $\Delta T_3$ 分别对应代码中的 `HEATER_BOOST_Y1`、`HEATER_BOOST_Y2` 和 `HEATER_BOOST_Y3`。
...
| $y_c(t)$ | `average(y and phi < 0)` | 气泡质心高度 |
